// =============================================
// ISYERI LISTESI EKRANI
// Tablo formatinda, kolon secici, filtreleme
// =============================================
//
// 📚 DERS: Firma liste ekraninin aynisi, isyeri alanlari ile.
// Farklar:
// - firma_adi kolonu ek olarak var (hangi firmaya ait)
// - Kolon tanimlari isyerine ozel
// - API cagrilari isyeri endpoint'lerine gider
...
import 'isyeri_form_screen.dart';

// 📚 DERS: Kolon tanimi (firma_list_screen'deki ile ayni)
class KolonTanimi {
...
class IsyeriListScreen extends StatefulWidget {
  const IsyeriListScreen({super.key});

  @override
  State<IsyeriListScreen> createState() => _IsyeriListScreenState();
}

class _IsyeriListScreenState extends State<IsyeriListScreen> {
  final _apiService = ApiService();

  // ---- KOLON TANIMLARI ----
  static const List<KolonTanimi> _kolonlar = [
    KolonTanimi(anahtar: 'ad', baslik: 'Isyeri Adi', varsayilanGorunum: true, genislik: 200),
    KolonTanimi(anahtar: 'firma_adi', baslik: 'Firma', varsayilanGorunum: true, genislik: 180),
    KolonTanimi(anahtar: 'sgk_sicil_no', baslik: 'SGK Sicil No', varsayilanGorunum: true, genislik: 130),
    KolonTanimi(anahtar: 'nace_kodu', baslik: 'NACE Kodu', genislik: 100),
    KolonTanimi(anahtar: 'tehlike_sinifi', baslik: 'Tehlike Sinifi', varsayilanGorunum: true, genislik: 130),
    KolonTanimi(anahtar: 'isveren_ad', baslik: 'Isveren Adi', genislik: 130),
    KolonTanimi(anahtar: 'isveren_soyad', baslik: 'Isveren Soyadi', genislik: 130),
    KolonTanimi(anahtar: 'lokasyon', baslik: 'Lokasyon', genislik: 200),
    KolonTanimi(anahtar: 'ana_faaliyet', baslik: 'Ana Faaliyet', genislik: 180),
    KolonTanimi(anahtar: 'nace_aciklama', baslik: 'NACE Aciklama', genislik: 200),
  ];

  // Durum degiskenleri
  List<dynamic> _isyerleri = [];
  int _toplam = 0;
  ...
  List<dynamic> get _filtreliIsyerleri {
    if (_kolonFiltreleri.isEmpty) return _isyerleri;
    return _isyerleri.where((isyeri) {
      for (final entry in _kolonFiltreleri.entries) {
        ...
        final deger = isyeri[kolonAdi]?.toString() ?? '';
  ...
  Future<void> _isyerleriYukle() async {
    ...
    try {
      final sonuc = await _apiService.isyeriListele(
        arama: _aramaMetni.isNotEmpty ? _aramaMetni : null,
      );

      setState(() {
        _isyerleri = sonuc['isyerleri'] as List<dynamic>;
  ...
  // 📚 DERS: Tehlike sinifini renkli badge olarak goster
  Widget _tehlikeSinifiBadge(String deger) {
    Color renk;
    String metin;
    switch (deger) {
      case 'az_tehlikeli':
    ...
    return Container(
      ...
      child: Text(metin, style: TextStyle(fontSize: 11, color: renk, fontWeight: FontWeight.w600)),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
            ...
            onSelected: (value) {
              setState(() {
                if (value == 'kaldir') {
                  ...
                  for (final f in _filtreliIsyerleri) {
      ...
      // Yeni isyeri ekleme butonu
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () async {
          final eklendi = await Navigator.push<bool>(
            context,
            MaterialPageRoute(
              builder: (_) => const IsyeriFormScreen(),
            ),
          );
          if (eklendi == true) _isyerleriYukle();
        },
        icon: const Icon(Icons.add),
        label: const Text('Yeni Isyeri'),
      ),
  ...
  // ISYERI TABLOSU
  // =============================================
  Widget _isyeriTablosu() {
    final aktifKolonlar = _kolonlar
        .where((k) => _gorunurKolonlar.contains(k.anahtar))
        .toList();

    final gosterilecek = _filtreliIsyerleri;
            ...
            child: Row(
              children: [
                ...
                if (_kolonFiltreleri.isNotEmpty)
                  Text(
                    'Filtre: ${gosterilecek.length}/${_isyerleri.length}',
  ...
  Widget _duzTablo(List<KolonTanimi> aktifKolonlar, List<dynamic> isyerleri) {
    return ListView.builder(
      itemCount: isyerleri.length,
      itemBuilder: (context, index) {
        return _isyeriSatiri(aktifKolonlar, isyerleri[index], index + 1);
      },
    );
  }
  ...
  // GRUPLU TABLO
  // =============================================
  Widget _grupluTablo(List<KolonTanimi> aktifKolonlar, List<dynamic> isyerleri) {
    final gruplar = <String, List<dynamic>>{};
    for (final iy in isyerleri) {
      final deger = iy[_grupKolonu]?.toString() ?? '';
      final grupAdi = deger.isEmpty ? '(Bos)' : deger;
      gruplar.putIfAbsent(grupAdi, () => []).add(iy);
    }
    ...
    for (final grupAdi in grupIsimleri) {
      final grupIsyerleri = gruplar[grupAdi]!;
      satirlar.add(_TabloSatir(tip: _SatirTip.grupBaslik, grupAdi: grupAdi, grupSayisi: grupIsyerleri.length));
      if (_acikGruplar.contains(grupAdi)) {
        for (final iy in grupIsyerleri) {
          satirlar.add(_TabloSatir(tip: _SatirTip.veri, isyeri: iy, siraNo: siraNo++));
        }
      }
    }
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return _isyeriSatiri(aktifKolonlar, satir.isyeri!, satir.siraNo!);
      },
    );
  }
        ...
        child: Row(
          children: [
            ...
            ...aktifKolonlar.map((k) {
              ...
              // Tehlike sinifi ozel gosterim
              if (k.anahtar == 'tehlike_sinifi' && deger.isNotEmpty) {
                return Expanded(
                  child: Padding(
                    padding: const EdgeInsets.only(right: 8),
                    child: _tehlikeSinifiBadge(deger),
                  ),
                );
              }
            ...
            SizedBox(
              ...
              child: PopupMenuButton<String>(
                ...
                onSelected: (value) async {
                  if (value == 'duzenle') {
                    _isyeriDuzenle(isyeri);
                  } else if (value == 'sil') {
                    _isyeriSilOnay(isyeri);
                  } else if (value == 'log') {
                    _kayitLogGoster(isyeri['id'], isyeri['ad']);
                  }
                },
                itemBuilder: (context) => [
                  const PopupMenuItem(
  ...
  void _kolonFiltreDialogGoster(KolonTanimi kolon) {
    ...
    for (final iy in _isyerleri) {
      final deger = iy[kolon.anahtar]?.toString() ?? '';
  ...
  // Isyeri duzenleme
  Future<void> _isyeriDuzenle(Map<String, dynamic> isyeri) async {
    final guncellendi = await Navigator.push<bool>(
      context,
      MaterialPageRoute(
        builder: (_) => IsyeriFormScreen(isyeri: isyeri),
      ),
    );
    if (guncellendi == true) _isyerleriYukle();
  }
        ...
        children: [
          ...
          Text(
            _aramaMetni.isNotEmpty
                ...
                : 'Henuz isyeri eklenmemis',
            style: TextStyle(color: Colors.grey[600], fontSize: 16),
          ),
          const SizedBox(height: 8),
          if (_aramaMetni.isEmpty)
            Text(
              'Sag alttaki + butonuyla yeni isyeri ekleyin',
  ...
  Future<void> _excelExport() async {
    try {
      ...
      final bytes = await _apiService.isyeriExcelExport(
        arama: _aramaMetni.isNotEmpty ? _aramaMetni : null,
      );
      ...
      html.AnchorElement(href: url)
        ..setAttribute('download', 'isyerleri.xlsx')
        ..click();
  ...
  Future<void> _excelSablonIndir() async {
    try {
      final bytes = await _apiService.isyeriExcelSablon();
      final blob = html.Blob([Uint8List.fromList(bytes)],
          'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
      final url = html.Url.createObjectUrlFromBlob(blob);
      html.AnchorElement(href: url)
        ..setAttribute('download', 'isyeri_sablon.xlsx')
        ..click();
      html.Url.revokeObjectUrl(url);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _excelImport() async {
    try {
      ...
      final sonuc = await _apiService.isyeriExcelImport(bytes, file.name);
      if (mounted) {
  ...
  // Isyeri silme onay
  void _isyeriSilOnay(Map<String, dynamic> isyeri) {
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('Isyeri Sil'),
        content: Text("'${isyeri['ad']}' isyerini silmek istiyor musunuz?"),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx),
            child: const Text('Iptal'),
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              Navigator.pop(ctx);
              try {
                await _apiService.isyeriSil(isyeri['id']);
                _isyerleriYukle();
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text("'${isyeri['ad']}' silindi")),
                  );
                }
              } catch (e) {
                if (mounted) {
